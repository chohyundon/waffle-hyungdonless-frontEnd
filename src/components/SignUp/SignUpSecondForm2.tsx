...
export const SignUpSecondForm = () => {
  ...
  const { count, startTimer, seconds, formatTime, format } = useTimer();
  ...
  return (
    <form
      ...
      onSubmit={handleSubmit(sendSignUpStep2Value)}
    >
      ...
      />
      {errors.name && (
        <span className={styles.errorFont}>{errors.name.message}</span>
      )}
      ...
      {errors.birth && (
        <span className={styles.errorFont1}>{errors.birth.message}</span>
      )}
      ...
      {errors.number && (
        <span className={styles.errorFont2}>{errors.number.message}</span>
      )}
      <input
        type='text'
        className={styles.inputBoxExample}
        aria-invalid={
          isSubmitted ? (errors.checkNumber ? 'true' : 'false') : undefined
        }
        placeholder='인증번호 입력'
        {...register('checkNumber', {
          required: '인증번호 입력은 필수 입니다.',
          validate: (value) =>
            value === checkNumberData || '인증번호가 일치하지 않습니다.',
        })}
      />
      ...
      {errors.checkNumber && (
        <span className={styles.errorFont3}>{errors.checkNumber.message}</span>
      )}
